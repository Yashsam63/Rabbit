...
const CheckOut = () => {
    ...
    const handleCreateCheckout = async (e) => {
        ...
        if (cart && cart.products.length > 0) {
            // map cart products to match backend schema
            const formattedCheckoutItems = cart.products.map(item => ({
                productId: item.productId,  // must be here
                name: item.name,
                images: item.images,
            ...
            console.log("Checkout Items:", formattedCheckoutItems);
            ...
            const res = await dispatch(createCheckout({
                ...
                shippingAddress,
                paymentMethod: "PayPal",   // note lowercase 'paymentMethod'
    ...
    if (error) return <p className='text-red-500'>Something went wrong: {error}</p>;
    if (!cart || !cart.products || cart.products.length === 0) {
        return <p>Your cart is empty. Please add items to your cart before proceeding to checkout.</p>;
    }
